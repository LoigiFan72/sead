...
namespace sead {
template <typename T>
inline T QuatCalcCtr<T>::length(const Base& v) 
{
    return sqrt(dot(v, v));
}

template <typename T>
inline T QuatCalcCtr<T>::normalize(Base& v) 
{
...
template <typename T>
inline T QuatCalcCtr<T>::dot(const Base& u, const Base& v) 
{
    return (u.w * v.w) + (u.x * v.x) + (u.y * v.y) + (u.z * v.z);
}

template <typename T>
inline void QuatCalcCtr<T>::setMul(Base& out, const Base& u, const Base& v) 
{
...
/*template <typename T>
inline void QuatCalcCtr<T>::slerpTo(Base& out, const Base& q1, const Base& q2, f32 t)
{
...
template <typename T>
inline void QuatCalcCtr<T>::makeUnit(Base& q) 
{
    q = Quat<T>::unit;
}

template <typename T>
inline bool QuatCalcCtr<T>::makeVectorRotation(Base& q, const Vec3& from, const Vec3& to) 
{
...
template <typename T>
inline void QuatCalcCtr<T>::set(Base& q, T w, T x, T y, T z) 
{
    q.w = w;
    q.x = x;
    q.y = y;
...
template <typename T>
inline void QuatCalcCtr<T>::setRPY(Base& q, T roll, T pitch, T yaw) 
{
