...
void PrimitiveRenderer::drawQuad(const QuadArg& arg)
{
    ...
    if (arg.isHorizontal())
    {
        mtx.makeSRT(Vector3f(arg.getSize().y, arg.getSize().x, 1.0f), Vector3f(0.0f, 0.0f, Mathf::deg2rad(90)), arg.getCenter());
    }
    else
    {
...
void PrimitiveRenderer::drawBox(const QuadArg& arg)
{
    ...
    if (arg.isHorizontal())
    {
        mtx.makeSRT(Vector3f(arg.getSize().x, arg.getSize().y, 1.0f), Vector3f(0.0f, 0.0f, Mathf::deg2rad(90)), arg.getCenter());
    }
    else
    {
        mtx.makeST(Vector3f(arg.getSize().x, arg.getSize().y, 1.0f), arg.getCenter());
...
PrimitiveRenderer::QuadArg& PrimitiveRenderer::QuadArg::setCornerAndSize(const Vector3f& p, const Vector2f& size)
{
    mCenter.set(p.x + size.x * 0.5f, p.y + size.y * 0.5f, p.z);
    mSize = size;
    return *this;
}
...
PrimitiveRenderer::QuadArg& PrimitiveRenderer::QuadArg::setColor(const Color4f& colorT, const Color4f& colorB)
{
    mHorizontal = false;
    mColor0 = colorT;
    mColor1 = colorB;
