...
u32 convertCalendarDateToDaysSince1970(const sead::CalendarTime::Date& date)
{
    ...
    if (m >= 3 && sead::DateUtil::isLeapYear(date.mYear.getValue()))
    ...
    if (date.mYear.getValue() > 1970)
    {
        ...
        do
        {
            num_days_since_1970 += sead::DateUtil::isLeapYear(year);
            ++year;
...
u64 convertCalendarTimeToSeconds(const sead::CalendarTime::Time& time)
{
    return 60 * (60 * time.mHour.getValue() + time.mMinute.getValue()) + time.mSecond.getValue();
}

u64 convertCalendarDateTimeToSeconds(const sead::CalendarTime::Date& date, const sead::CalendarTime::Time& time)
{
#ifdef SEAD_DEBUG
    const u32 y = date.mYear.getValue();
    ...
    u32 num_days;
    if (m == 2 && sead::DateUtil::isLeapYear(y))
...
u32 convertDaysToYears(u32* days)
{
    ...
    do
    {
        ++year;
        days_to_remove = i;
        i += sead::DateUtil::isLeapYear(year) ? 366 : 365;
    } while (i <= *days);
    *days -= days_to_remove;
    return year;
}
...
s32 convertDaysToMonth(u32* days, u32 year)
{
    ...
    {
        days_to_remove = i;
        i += (month_idx == 1 && sead::DateUtil::isLeapYear(year)) ? 29 : sDaysOfMonth[month_idx];
        if (*days < i)
            break;
        ++month_idx;
    } while (month_idx < 12);
...
u64 DateTime::setNow()
{
    ...
    const CalendarTime::Year year = CalendarTime::Year(ctime.GetYear());
    const CalendarTime::Month month = CalendarTime::Month::makeFromValueOneOrigin(ctime.GetMonth());
    const CalendarTime::Day day = CalendarTime::Day(ctime.GetDay());
    const CalendarTime::Hour hour = CalendarTime::Hour(ctime.GetHour());
    const CalendarTime::Minute minute = CalendarTime::Minute(ctime.GetMinute());
    const CalendarTime::Second second = CalendarTime::Second(ctime.GetSecond());
    setUnixTime(year, month, day, hour, minute, second);
...
void DateTime::getCalendarTime(CalendarTime* calendar) const
{
    ...
    const u64 reduced_time = mUnixTime % (3600 * 24);
